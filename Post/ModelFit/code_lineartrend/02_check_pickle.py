...
file = open(ftxt, "w")
content="Fitting_results,stnm,A,B,C,tshift,blin,logf,nsteps,nwlker\n"
...
ftxt="ModelCase_"+model+"_scalar_coef.txt"
file = open(ftxt, "w")
content="Fitting_results,stnm,A,B,blin,logf,nsteps,nwlker\n"
print("output content to scalar_coef.txt ---  ",content.strip())
file.write(content)
with open(fnst, 'r') as f:
    for stnm in f:
        ...
        #-------------------------------------------#
            ...
            temp=[" %.6f " % k for k in theta_max]
            coef=[float(string) for string in temp]
            content="Fitting_results,"+stnm.strip()+",%f,%f,%f,%f"%(coef[0],coef[1],coef[2],coef[3])+","+str(nsteps)+","+str(modelparam["nwalkers"])+"\n"
            print("output content to scalar_coef.txt ---  ",content.strip())
            file.write(content)
        fieldnames = ['utvec', 'dv','bestfit','Resi','sdv']
        fcsv="Fit_"+stnm.strip()+"_"+model+".csv"
...
ftxt="ModelCase_"+model+"_scalar_coef.txt"
file = open(ftxt, "w")
content="Fitting_results,stnm,A,C,tshift,blin,logf,nsteps,nwlker\n"
print("output content to scalar_coef.txt ---  ",content.strip())
file.write(content)
with open(fnst, 'r') as f:
    for stnm in f:
