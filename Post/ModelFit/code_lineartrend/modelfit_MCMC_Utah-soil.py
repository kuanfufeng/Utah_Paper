...
def sample_walkers(nsamples,flattened_chain):
    ...
    for i in thetas:
        mod = model_soil(i, all=False, **modelparam)
        models.append(mod)
...
if __name__ == '__main__':
    ...
    # model case
    modelparam["modelcase"] = "soil" 

    # MCMC parameters
    modelparam["nwalkers"] =  32 # number of chains

    #output_imgdir = "../figure/MCMC_modelfit"
    output_imgdir = "../figure/Fitdv/MCMC_soil"
    output_datadir = "../processed_fitdv_soil/MCMC_sampler_{}_Normalized".format(nsteps)
    ...
    for stationpair in tqdm([stationpairs[i] for i in stationids]):
        ...
        best_fit_model = model_soil(theta_max, all=False, **modelparam)

        fig, ax = plt.subplots(3, 1, figsize=(12,10))
        for theta in samples[np.random.randint(len(samples),size=9000)]:
            ax[0].plot(uniform_tvec, model_soil(theta, all=False, **modelparam), color="r", alpha=0.1)
