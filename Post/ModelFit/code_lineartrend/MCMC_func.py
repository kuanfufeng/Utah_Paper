...
def get_keys(modelcase):
    ...
    if modelcase.lower() == "soil":
        # model soil
        keys = ['A', 'B', 'b_{lin}', 'logf'] # only soil

    elif modelcase.lower() == "temp":
        # model with linear trend
        keys = ['A',  'C', 't_{shiftdays}', 'b_{lin}', 'logf'] # only temperature
        
    elif modelcase.lower() == "soil_temp":
        # model 
        keys = ['A', 'B', 'C', 't_{shiftdays}', 'b_{lin}', 'logf'] # soil +  temperature
        

    elif modelcase.lower() == "soil_temp_snow":
        # model 
        keys = ['A', 'B', 'C', 't_{shiftdays}', 'D', 'b_{lin}', 'logf'] # soil + temperature + snow
...
def model_temp(theta, all=False, **modelparam):
    ...
    A,  C, t_shiftdays, b_lin, log_f = theta
    ...
    # Construct model
    model = A + C * T_shift_trim + lintrend
    #---------------------#

    return model


def model_soil_temp_snow(theta, all=False, **modelparam):
    """
    dv/v soil, temp, snow model with linear trend term.
    """

    assert modelparam["ndim"] == len(theta)

    A, B, C, t_shiftdays, D, b_lin, log_f = theta
    
    # get parameters from dictionary
    unix_tvec          = modelparam["unix_tvec"]
    fitting_period_ind = modelparam["fitting_period_ind"]
    snow     = modelparam["snow"]
    ...
    # Construct model
    model = A + B * soil_trim + C * T_shift_trim + D * snow + lintrend
    #---------------------#
...
def model_soil_temp(theta, all=False, **modelparam):
    ...
    A, B, C, t_shiftdays, b_lin, log_f = theta
    ...
    # Construct model
    model = A + B * soil_trim + C * T_shift_trim + lintrend
    #---------------------#
...
def log_likelihood(theta, **modelparam):
    ...
    if modelcase.lower() == "temp":
        ...
        model = model_soil_temp_snow(theta, all=False, **modelparam)
