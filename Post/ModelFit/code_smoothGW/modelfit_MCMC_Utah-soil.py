...
if __name__ == '__main__':
    ...
    modelparam = {
                ...
                "B"            : (-1.0, [-np.inf, 0]), # scale of GWL mimic by soil moisture equivalent water thickness
    ...
    # model case
    modelparam["modelcase"] = "soil_lake" 

    # MCMC parameters
    modelparam["nwalkers"] =  32 # number of chains

    #output_imgdir = "../figure/MCMC_modelfit"
    output_imgdir = "../figure/Fitdv/MCMC_soil_lake"
    output_datadir = "../processed_fitdv_soil_lake/MCMC_sampler_{}_Normalized".format(nsteps)
    ...
    for stationpair in tqdm([stationpairs[i] for i in stationids]):
        ...
        ax[1].plot(uniform_tvec,modelparam["soil"],  ls="-", c = "b")
        ax[1].set_title("Soil Moisture EWT (m) from NLDAS")
        ax[2].plot(uniform_tvec,UL,  ls="-", c = "g")
        ax[2].plot(uniform_tvec,gwtemp,  ls="--", c = "lime")
        ax[2].set_title("Utah Lake Level (m) from USGS")
        plt.tight_layout()
        ...
        for theta in samples[np.random.randint(len(samples),size=9000)]:
            ax[0].plot(uniform_tvec, model_soil_lake(theta, all=False, **modelparam), color="r", alpha=0.1)
