...
if __name__ == '__main__':
    ...
    modelparam = {
                ...
                "B"            : (-0.1, [-np.inf, 0]), # scale of soil moisture equivalent water thickness
                "C"            : ( 0.1, [0, np.inf]), # scale of Temp
                "t_{shiftdays}"   : (1, [0, 90]), # shift of temp in days
                "D"            : (0.0, [-np.inf, 0]), # scale of lake lavel, assumption of ground water level
                "w_{shiftdays}"   : (1, [-182, 182]), # shift of gwl in days
                "logf"         : (0.0, [-10, 10]), # magnitude of uncertainity
                }
                #"b_{lin}"         : (0.0, [0, np.inf]), # slope of linear trend
    # model case
    modelparam["modelcase"] = "soil_temp_lake" # "temp" "base" or "wlin"

    # MCMC parameters
    modelparam["nwalkers"] =  32 

    #output_imgdir = "../figure/MCMC_test"
    output_imgdir = "../figure/Fitdv/MCMC_soil_temp_lake"
    output_datadir = "../processed_fitdv_soil_temp_lake/MCMC_sampler_{}_Normalized".format(nsteps)
    ...
    modelparam["soil"]=(np.array(fi['soil'])-np.mean(np.array(fi['soil'])))
    modelparam["soil"]=modelparam["soil"]/np.max(np.abs(modelparam["soil"]))
    ...
    gwtemp=(np.array(fi['UL'])-np.mean(np.array(fi['UL'])))
    gwtemp=lowpass(gwtemp, freq=corner_frequency, df=sampling_rate, corners=order, zerophase=True)
    modelparam["GW"]=gwtemp/np.max(np.abs(gwtemp))
    UL=np.array(fi['UL'])-np.array(fi['UL']).mean()
    ...
    for stationpair in tqdm([stationpairs[i] for i in stationids]):
        ...
        ax[1].plot(uniform_tvec, np.array(fi['temp']),  ls="-", c = "orange")
        ax[0].set_title(stationpair)
        ax[1].set_title("Temperature (C) from PRISM")
        ax[2].plot(uniform_tvec,np.array(fi['soil']),  ls="-", c = "b")
        ax[2].set_title("Soil Moisture EWT (m) from NLDAS")
        ax[3].plot(uniform_tvec,UL,  ls="-", c = "g")
        ax[3].plot(uniform_tvec,gwtemp,  ls="--", c = "lime")
        ax[3].set_title("Utah Lake Level (m) from USGS")
        plt.tight_layout()
        ...
        for theta in samples[np.random.randint(len(samples),size=9000)]:
            ax[0].plot(uniform_tvec, model_soil_temp_lake(theta, all=False, **modelparam), color="r", alpha=0.1)
        ...
        plt.tight_layout()
        plt.savefig(output_imgdir+"/Samp_MCMCdvv_%s_%s_samplers.png"%(stnm, freqband), format="png", dpi=100)
        plt.close()
        plt.clf()  
        ...
        axes[0].set_title(stnm+" --> theta_max: "+str(title))
