...
if __name__ == '__main__':
    ...
    # model case
    modelparam["modelcase"] = "temp_lake" 

    # MCMC parameters
    modelparam["nwalkers"] =  32 # number of chains

    #output_imgdir = "../figure/MCMC_modelfit"
    output_imgdir = "../figure/Fitdv/MCMC_temp_lake"
    output_datadir = "../processed_fitdv_temp_lake/MCMC_sampler_{}_Normalized".format(nsteps)
    ...
    for stationpair in tqdm([stationpairs[i] for i in stationids]):
        ...
        ax[1].set_title("Temp degree C from PRISM")
        ...
        for theta in samples[np.random.randint(len(samples),size=9000)]:
            ax[0].plot(uniform_tvec, model_temp_lake(theta, all=False, **modelparam), color="r", alpha=0.1)
